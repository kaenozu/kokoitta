...
void main() {
  ...
  testWidgets(
    '360 width and 200 percent text remain scrollable without overflow',
    (tester) async {
      await pumpView(tester, size: const Size(360, 800), textScale: 2);
  ...
  testWidgets('dark tablet layout keeps backup and restore actions', (
    tester,
  ) async {
    await pumpView(tester, size: const Size(800, 1200), dark: true);
    expect(find.text('完全バックアップを作成'), findsOneWidget);
  ...
  testWidgets('busy state disables conflicting actions and explains why', (
    ...
    await pumpView(tester, size: const Size(412, 915), busy: true);
  ...
  testWidgets(
    'backup and restore actions expose labels and execute callbacks',
    (tester) async {
      var backups = 0;
      var restores = 0;
      await tester.pumpWidget(
      ...
      expect(find.bySemanticsLabel('完全バックアップを作成'), findsAtLeastNWidgets(1));
