...
void main() {
  ...
  group('final UI QA matrix', () {
    testWidgets('settings supports viewport theme and text-scale matrix', (
      ...
      final brightnesses = <Brightness>[Brightness.light, Brightness.dark];
    ...
    testWidgets(
      'state tones remain explicit and readable without color alone',
      (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            theme: buildKokoittaTheme(Brightness.light),
        ...
        for (final title in <String>['読み込み中', '完了', '一部失敗', 'エラー', '保存上限']) {
    ...
    testWidgets(
      'primary settings actions meet tap-target and callback contract',
      (tester) async {
        var backupCalls = 0;
        var restoreCalls = 0;
        await tester.pumpWidget(
        ...
        final backupTile = find.widgetWithText(ListTile, '完全バックアップを作成');
